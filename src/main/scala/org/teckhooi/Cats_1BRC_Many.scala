...
object Cats_1BRC_Many extends IOApp:
  def openFile[F[_]: Sync](filename: String): F[BufferedReader] =
    Sync[F].blocking(BufferedReader(FileReader(filename)))

  def readLine[F[_]: Async](reader: BufferedReader): F[Map[String, City]] =
    def _readLine(
        reader: BufferedReader,
        acc: Ref[F, Map[String, City]]
    ): F[Map[String, City]] =
      for
        lineF <- Async[F].start(Async[F].blocking(reader.readLine()))
        line <- lineF.joinWithNever
        result <- Option(line).fold(acc.get)(s =>
          val ndx  = s.indexOf(';')
          val name = s.substring(0, ndx)
          var curr = line.substring(ndx + 1).toDouble
          acc
            .update(m =>
              m + (name -> m
                .get(name)
